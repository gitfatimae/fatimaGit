...
public class GetOffersTest {
   ...
   // private static final Task.Money OFFER_PRICE = new Task.Money(OFFER_PRICE_CURRENCY, OFFER_PRICE_AMOUNT);
    ...
    private UUID addOffer(String name, LocalDate endDate) {
        return database.offerGateway().add(new Task(name, GetOffersTest.OFFER_DESCRIPTION, GetOffersTest.OFFER_START_DATE, endDate));
    }
    ...
    public void cancelled_offers_do_not_appear_in_list() {
        ...
        UUID offer1Id = addOffer("offer-1", OFFER_END_DATE);
        UUID offer2Id = addOffer("offer-2", OFFER_END_DATE);
        Task offer2 = database.offerGateway().getByIdExcludingCancelled(offer2Id);
